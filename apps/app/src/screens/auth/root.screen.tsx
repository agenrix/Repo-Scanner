    ...
    <main className="grid min-h-svh bg-background text-foreground lg:grid-cols-[minmax(0,1.08fr)_minmax(420px,0.92fr)]">
      <div className="relative hidden min-h-svh overflow-hidden border-border border-r bg-card lg:block">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="absolute bottom-5 left-5 font-medium text-muted-foreground text-xs transition-colors duration-300 hover:text-foreground/65"
        >
          Image by Sumit (rightfit.so)
        </a>
      </div>

      <section className="mx-auto flex min-h-screen w-1/2 flex-col items-center justify-center">
        <Outlet />
      </section>
    </main>
  );
