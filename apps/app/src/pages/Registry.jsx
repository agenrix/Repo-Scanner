...
export default function Registry() {
  ...
  Object.values(COLUMNS_CATEGORIES)
    .flat()
    .forEach((col) => {
      initialColumns[col.key] = [
        "id",
        "name",
        "repo_id",
        "repo_link",
  ...
  const buildFlatData = (item) => {
    ...
    return {
      ...
      session_id: [
        ...new Set(nosql_logs.map((l) => l.event?.session_id).filter(Boolean)),
      ].join(", "),
      used_by: [
        ...new Set(nosql_logs.map((l) => l.event?.used_by).filter(Boolean)),
      ].join(", "),
      action: [
        ...new Set(nosql_logs.map((l) => l.event?.action).filter(Boolean)),
      ].join(", "),
      files_altered: [
        ...new Set(
          nosql_logs
            .flatMap((l) => l.event?.files_altered || [])
            .filter(Boolean),
        ),
      ].join(", "),
      recipient: [
        ...new Set(nosql_logs.map((l) => l.recipient).filter(Boolean)),
      ].join(", "),
      item: [
        ...new Set(
          nosql_logs
            .flatMap((l) => (l.data_shared || []).map((d) => d.item))
            .filter(Boolean),
        ),
      ].join(", "),
      comm_classification: [
        ...new Set(
          nosql_logs
            .flatMap((l) => (l.data_shared || []).map((d) => d.classification))
            .filter(Boolean),
        ),
      ].join(", "),
      encryption_status: [
        ...new Set(
          nosql_logs
            .flatMap((l) =>
              (l.data_shared || []).map((d) => d.encryption_status),
            )
            .filter(Boolean),
        ),
      ].join(", "),
              ...
              {paramMenuOpen && (
                <div className="absolute top-full left-0 mt-2 w-72 bg-card border border-border rounded-md shadow-xl z-50 p-2 max-h-[60vh] overflow-y-auto">
                  {Object.entries(CATEGORIES).map(([catName, params]) => {
                    const allSelected = params.every(
                      (p) => p.key in activeParams,
                    );
                    return (
                      <div key={catName} className="mb-2">
                        ...
                        >
                          <span className="text-xs font-bold text-foreground/70 uppercase">
                            {catName}
                          </span>
                          {allSelected ? (
                    ...
                    return (
                      <div key={catName} className="mb-2">
                        <div
                          className="flex items-center justify-between px-3 py-1.5 bg-muted/50 rounded cursor-pointer hover:bg-muted"
                          onClick={() =>
                            handleCategorySelectAllColumns(catName)
                          }
                        >
                          <span className="text-xs font-bold text-foreground/70 uppercase">
                            {catName}
        ...
        {Object.keys(activeParams).length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mt-2 pt-4 border-t border-border">
            {Object.keys(activeParams).map((key) => {
              const paramDef = Object.values(CATEGORIES)
                .flat()
                .find((p) => p.key === key);
              return (
                <div
                ...
                >
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={(e) =>
                        handleParamValueChange(key, e.target.value)
                      }
                      onKeyDown={(e) =>
                        e.key === "Enter" && fetchRegistryData(true)
                      }
            ...
            <thead className="bg-muted/50 dark:bg-card border-b border-border">
              <tr>
                {Object.entries(COLUMNS_CATEGORIES).map(([cat, cols]) => {
                  const visibleCount = cols.filter(
                    (c) => visibleColumns[c.key],
                  ).length;
                  ...
                  return (
                    <th
                      ...
                      className="p-2 border-b border-r border-border text-center font-bold text-[10px] uppercase bg-muted/80 dark:bg-card text-foreground tracking-wider last:border-r-0"
                    >
                      {cat}
                    </th>
                  );
                ...
                {Object.entries(COLUMNS_CATEGORIES).map(([cat, cols]) =>
                  cols
                    ...
                    .map((col) => (
                      <th
                        key={col.key}
                        className="p-4 font-semibold text-xs text-foreground/70 border-r border-border last:border-r-0 whitespace-nowrap"
                      >
                        {col.label}
                      </th>
                    )),
                          ...
                          .map((col) => {
                            ...
                            if (col.key === "repo_link" && val) {
                              return (
                                <td
                                  key={col.key}
                                  className="p-4 max-w-[200px] truncate text-sm border-r border-border/50 last:border-r-0"
                                >
                                  <a
                                    href={val}
                                    target="_blank"
                            ...
                            if (col.key === "repo_classification") {
                              return (
                                <td
                                  key={col.key}
                                  className="p-4 text-sm border-r border-border/50 last:border-r-0"
                                >
                                  <span
                            ...
                            return (
                              <td
                                key={col.key}
                                className={`p-4 max-w-[200px] truncate text-sm text-foreground/80 border-r border-border/50 last:border-r-0 ${
                                  [
                                    "id",
                                    "name",
                                    "source_repo_id",
