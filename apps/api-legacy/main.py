...
from config.db_sql import Base, engine, get_sql_db
...
@app.post("/add_agent")
async def add_agent(agent: AgentCreate, db: AsyncSession = Depends(get_sql_db)):
    try:
        ...
        return {
            ...
            "data": {"id": new_agent.id, "agent_id": new_agent.agent_id},
        }
...
async def list_agents(
        ...
        if repo_sql_filter_active:
            ...
            if repo_name:
                repo_query = repo_query.where(
                    RepoModelSQL.repo_name.ilike(f"%{repo_name}%")
                )
            if repo_link:
                repo_query = repo_query.where(
                    RepoModelSQL.repo_link.ilike(f"%{repo_link}%")
                )
            if repo_classification:
                repo_query = repo_query.where(
                    RepoModelSQL.classification == repo_classification
                )
            if repo_confidence:
                repo_query = repo_query.where(
                    RepoModelSQL.confidence == repo_confidence
                )
    ...
    if source_repo_id and str(source_repo_id).strip():
        query = query.where(
            AgentModel.source_repo_id.ilike(f"%{str(source_repo_id).strip()}%")
        )
    ...
    if subscription_plan and str(subscription_plan).strip():
        query = query.where(
            AgentModel.subscription_plan == str(subscription_plan).strip()
        )
    ...
    if contributor and str(contributor).strip():
        query = query.where(
            AgentModel.contributors.contains([str(contributor).strip()])
        )

    if file and str(file).strip():
        query = query.where(
            AgentModel.access_rights["files"].contains([str(file).strip()])
        )

    if tool and str(tool).strip():
        query = query.where(
            AgentModel.access_rights["tools"].contains([str(tool).strip()])
        )

    if data_node and str(data_node).strip():
        query = query.where(
            AgentModel.access_rights["data_nodes"].contains([str(data_node).strip()])
        )

    if api and str(api).strip():
        query = query.where(
            AgentModel.access_rights["apis"].contains([str(api).strip()])
        )

    if server and str(server).strip():
        query = query.where(
            AgentModel.access_rights["servers"].contains([str(server).strip()])
        )
    ...
    async def fetch_repo_data(agent):
        try:
            ...
            return (
                jsonable_encoder(r_sql) if r_sql else None,
                r_nosql[0] if r_nosql else None,
            )
        except Exception:
            return None, None
...
async def update_agent(
    agent_id: str, update_data: dict, db: AsyncSession = Depends(get_sql_db)
):
...
async def add_repo_scan(
    ...
    try:
        ...
        if scan_data.classification in [
            ClassificationEnum.AGENT,
            ClassificationEnum.POSSIBLE_AGENT,
        ]:
            if not payload.agent or not payload.agent.agent_id:
                raise HTTPException(
                    status_code=400,
                    detail="agent_id inside agent payload is mandatory if classification is AGENT or POSSIBLE_AGENT",
                )
            ...
            if not agent:
                try:
                    new_agent = AgentModel(
                        ...
                        access_rights=payload.agent.access_rights
                        or {
                            "files": [],
                            "tools": [],
                            "data_nodes": [],
        ...
        if payload.agent:
            background_tasks.add_task(
                NoSQLModel.add_telemetry_record, payload.agent.model_dump()
            )
...
async def list_repo_scans(
    ...
    try:
        ...
        if nosql_filter_active:
            ...
            matching_repo_ids = list(
                set([r["repo_id"] for r in nosql_records if "repo_id" in r])
            )
            if not matching_repo_ids:
                return []
        ...
        if repo_name and str(repo_name).strip():
            query = query.where(
                RepoModelSQL.repo_name.ilike(f"%{str(repo_name).strip()}%")
            )
        if repo_link and str(repo_link).strip():
            query = query.where(
                RepoModelSQL.repo_link.ilike(f"%{str(repo_link).strip()}%")
            )
        if classification and str(classification).strip():
            query = query.where(
                RepoModelSQL.classification == str(classification).strip()
            )
        if confidence and str(confidence).strip():
        ...
        for repo, nosql in zip(repos, nosql_results):
            ...
            response.append({"sql_data": repo_dict, "nosql_data": nosql})
